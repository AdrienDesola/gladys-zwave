    ...
    return gladys.param.getValue(shared.gladysUsbPortParam)
        .then((usbPort) => {
            ...
            zwave.on('node ready', function(nodeid, nodeinfo) {
                ...
                // add the scene type so that scene controller can throw scenes
                newDevice.types.push({
                    name: 'Scene',
                    type: 'scene',
                    identifier: 'scene' + nodeid,
                    sensor: true,
                    min: 0,
                    max: 32767,
                    display: false
            ...
            zwave.on('scene event', function(nodeid, sceneid){
                gladys.deviceState.createByDeviceTypeIdentifier('scene' + nodeid, 'zwave', {value: sceneid})
                      .catch((err) => sails.log.warn(`Zwave Module : Fail to save deviceState : ${err}`));
            });
