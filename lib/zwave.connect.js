    ...
    return gladys.param.getValue(shared.gladysUsbPortParam)
        .then((usbPort) => {
            ...
            zwave.on('value added', function(nodeid, comclass, value) {
                ...
                gladys.deviceState.createByDeviceTypeIdentifier(value.value_id, 'zwave', state)
                ...
                for (comclass in nodes[nodeid]['classes']) {
                    ...
                    for (idx in values) {
                        ...
                        newDevice.types.push({
                            ...
                            type: type,
                            identifier: values[idx].value_id,
                            sensor: values[idx].read_only,
                            unit: `${values[idx].label} ${values[idx].units}`,
                            min: min,
                            max: max,
                            display: false
                        });
            ...
            zwave.on('scene event', function(nodeid, sceneid){
                sails.log.info(`Node event : ${nodeid} ${sceneid}`);
            });
